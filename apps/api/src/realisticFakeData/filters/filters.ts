...
function validateFilter({ entity, entityName, fakeDataSource, rule }: ValidateFilter): boolean {
  ...
  switch (rule.operation) {
    ...
    case FilterOperation.IsNot:
      return validateSingleIsNotOperation({
        entity: attributeToFilter as FakeDataAttribute,
        ruleValue: rule.value,
      });
...
function validateSingleIsNotOperation({ entity, ruleValue }: SingleOperation): boolean {
  if (Array.isArray(ruleValue)) {
    return ruleValue.some(({ displayValue, id }) => {
      const ruleHasDisplayValue = !isNilOrEmpty(displayValue);
      const ruleHasId = !isNilOrEmpty(id);
      const displayValueNotMatches = ruleHasDisplayValue && entity.displayValue !== displayValue;
      const idNotMatches = ruleHasId && entity.id !== id;
      return displayValueNotMatches || idNotMatches;
    });
  }

  const ruleHasDisplayValue = !isNilOrEmpty(ruleValue.displayValue);
  const ruleHasId = !isNilOrEmpty(ruleValue.id);
  const displayValueNotMatches = ruleHasDisplayValue && entity.displayValue !== ruleValue.displayValue;
  const idNotMatches = ruleHasId && entity.id !== ruleValue.id;
  return displayValueNotMatches || idNotMatches;
}
