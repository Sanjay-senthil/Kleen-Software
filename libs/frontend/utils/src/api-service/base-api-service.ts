...
export class BaseApiService {
  ...
  static getErrorHandler(className: string, methodName: string) {
  ...
  static commonQueryFactory(entityName, queryToApply: (entity: string) => string, operationName: string) {
    return (params: string | { [key: string]: unknown }) => {
  ...
  static graphqlEntityCRUD(entityName: string) {
    return {
      create: BaseApiService.commonQueryFactory(entityName, createEntityQuery, `create${entityName}`),
      read: (params: { [key: string]: unknown }) => {
        const stringifiedId = String(params?.id);
      ...
      delete: (params: { [key: string]: unknown }) => {
        const stringifiedId = String(params?.id);
  ...
  static graphqlEntityAutoComplete(entityName: string) {
    return BaseApiService.commonQueryFactory(
      entityName,
      autoCompleteQuery,
  ...
  static graphqlFormatCheck({ formField, formValue, objectFocus, taskName, widgetId }) {
    return BaseApiService.commonQueryFactory(
      null,
      formatCheckQuery,
      'formatCheck',
    )({ formField, formValue, objectFocus, taskName, widgetId });
  }
}
