 ...
 */
export const parseStringifyToJson = <T extends { name: string; id: number | string }[]>({
  value = null,
  defaultValue,
}: {
  value?: any;
...
export const validateIsOrIsNorFilterOperator = (operator: string): boolean =>
  operator === FilterOperators.in || operator === FilterOperators.notIn;
...
export const sortByName = <T extends { name?: Maybe<string> }>(a: T, b: T) =>
