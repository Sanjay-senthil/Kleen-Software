...
import { validateInputValue } from '../../input-element/utils';
...
export function ConfigInputWidget({
  ...
  const getElementBasedOnTransformationAndType = (transformationLocal, type) => {
    switch (transformationLocal) {
  ...
  const getAttrValue = (): unknown => {
    const path = ['data', 'data', '0', attrProp];
    if (hasMany) {
      return pathOr<unknown>(INITIAL_ATTRIBUTE_VALUE_HAS_MANY, path, widgetData);
    }
  ...
  useEffect(() => {
    ...
    validateInputValue(inputValue, validateFormField);
  }, [inputValue]);
