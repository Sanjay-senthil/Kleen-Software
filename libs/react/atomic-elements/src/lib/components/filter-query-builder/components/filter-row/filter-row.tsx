...
function FilterRowAttributes({ classes, filterRule, index }: FilterRowPartialProps) {
  ...
  const sortedAttributes = attributes.map((attribute) => attribute.name).sort();
...
function FilterRowInput({ filterRule, index }: FilterRowPartialProps) {
  ...
  if (isFilterIdOnly(filterRule)) {
    return (
      <DisplayIdFilter field={filterRule.field} value={(filterRule.value as SingleFilterRuleValue).id} />
    );
  }
...
function DisplayIdFilter({ field, value: id }: DisplayIdFilterProps) {
  const [displayValue, setDisplayValue] = useState('');
  const [fieldId, setFieldId] = useState<number>();
  const classes = useStyles();

  useEffect(() => {
    const thing = getThingByName(field);
    if (!isNilOrEmpty(thing)) {
      setFieldId(thing.id);
    }
  }, [field]);
  ...
  useEffect(() => {
    if (!isNilOrEmpty(fieldId)) {
      BaseApiService.graphqlEntityCRUD(field)
        .read({
          baseModel: field,
          id,
        })
        .toPromise()
        .then(({ response }) => {
          const valueToShow = response?.data[`get${fieldId}`]?.data[field]?.displayValue || id;
  ...
  return (
    <Tooltip title={displayValue}>
      <span className={classes.displayOnlyField}>{displayValue}</span>
    </Tooltip>
  );
}
//#endregion
