import { FILTERS_KEY, FilterOperators, IntervalDate, TimestampKey } from '@kleeen/types';
import {
...
import { useOldLocalStorage as useLocalStorage, useUserInfo } from '@kleeen/react/hooks';
...
export const useFilters = (hasDateFilter = false) => {
  ...
  const filtersKey = userName ? `${FILTERS_KEY}${navigation.location.pathname}-${userName}` : null;
  const { localStorageValue, removeLocalStorageValue, setLocalStorageValue } = useLocalStorage<
    Record<string, any>
  >(filtersKey, '');
