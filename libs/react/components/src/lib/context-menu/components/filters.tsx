import { ContextMenuItem, DataPointWithFormattedValue, FILTERS_KEY, FilterTypes } from '@kleeen/types';
...
export function KsContextMenuFilterSection({ dataPointsToShow, handleClose }: ContextMenuSectionProps) {
  ...
  useEffect(() => {
    if (isNilOrEmpty(filtersKey) || isNilOrEmpty(availableDataPoints)) {
      return;
    }
  ...
  }, [filtersInUse, dataPointsToShow?.length, filtersKey, hasFilters]);
