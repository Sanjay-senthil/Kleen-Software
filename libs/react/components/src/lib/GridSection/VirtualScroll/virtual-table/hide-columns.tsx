...
import { SwitchHideColumn, Tooltip, useStyles } from './hide-column.style';
...
const HideColumns = (props: HideColumnsProps) => {
  ...
  function handleToggle(columnToShow: ColumnToShowArgs, index: number, e: SyntheticEvent) {
    e.stopPropagation();
    e.preventDefault();
            ...
            <div>
              ...
              <FormGroup>
                {columns.map((value, index) => {
                  return (
                    <Tooltip arrow enterDelay={400} placement="top" title={value.label}>
                      <FormControlLabel
                        className={classes.label}
                        control={
                          <SwitchHideColumn defaultChecked={!value.hideColumn} checked={!value.hideColumn} />
                        }
                        label={upperFirst(value.label)}
                        onClick={(event) => handleToggle(value, index, event)}
                      />
                    </Tooltip>
                  );
                })}
