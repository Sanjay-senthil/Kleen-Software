...
const useContextMenuInteractions = () => {
  ...
  function onCellClick(dataPointsParam: ContextMenuDataPoint[], hasCrossLinking: boolean) {
    const [dataPointParam] = dataPointsParam;
    const { attribute, value } = dataPointParam;
...
function ReactVirtualizedTableComponent({
  ...
  const { anchorEl, dataPoints, handleCloseHelper, onClickFunction, onContextMenuFunction, showContextMenu } =
