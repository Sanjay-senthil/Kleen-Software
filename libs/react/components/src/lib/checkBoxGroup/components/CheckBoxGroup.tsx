import { CbOptionsPayload, ICheckBoxGroupProps, ICheckBoxOption } from '../CheckBoxGroup.model';
...
export function CheckBoxGroupComponent(props: ICheckBoxGroupProps): JSX.Element {
  const options: CbOptionsPayload = pathOr({}, ['options'], props);
  const initialCheckedValues: ICheckBoxOption[] = pathOr([], ['initialCheckedValues'], props);
  const onChange: (arg: { displayValue: unknown }[]) => void = path(['onChange'], props);
  ...
  let formattedOptions: Maybe<{ value: string; label: string }[]> = [];
  ...
  useEffect(() => {
    const initialCheckedUniqueValues: string[] = [];
  ...
  const handleSingleBoxChange = (event: ChangeEvent<HTMLInputElement>): void => {
     ...
     */
    const formattedWithDisplayNameValues: ICheckBoxOption[] = [];
    ...
    if (onChange) onChange(formattedWithDisplayNameValues);
  };
