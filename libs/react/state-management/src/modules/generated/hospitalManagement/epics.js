...
import { catchErrorsFromRequest } from '../../../utils';
import { v4 as uuidv4 } from 'uuid';
import { actions as ActionsStructure } from './slice';
import { actions as ActionsNotifications } from '../../custom/notifications';
import { actions as ActionsGlobalAppErrors } from '../../custom/globalAppErrors';

const notificationActions = ActionsNotifications.actions;
const actions = ActionsStructure.actions;
const globalAppErrorsActions = ActionsGlobalAppErrors.actions;

/**
 * Add
 * @desc Redux-Observable Epic which allows to add an entity in "hospitalManagement" workflow.
 */
        ...
        .pipe(
          ...
          catchError(({ error }) => {
            ...
            return ofErrors.pipe(
              mergeMap((error) => [
                actions.addFailure(error),
                globalAppErrorsActions.dispatchCustomAction(error),
              ]),
            );
          }),
        );
...
/**
 * Bulk Add
 * @desc Redux-Observable Epic which allows to add entities in bulk in "hospitalManagement" workflow.
 */
...
/**
 * Delete
 * @desc Redux-Observable Epic which allows to delete an entity in "hospitalManagement" workflow.
 */
...
/**
 * Bulk Delete
 * @desc Redux-Observable Epic which allows to delete entities in bulk in "hospitalManagement" workflow.
 */
...
/**
 * Safe Delete
 * @desc Redux-Observable Epic which allows to safely delete entities in "hospitalManagement" workflow.
 */
export function safeDelete(action$) {
  return action$.pipe(
    ofType(actions.safeDeleteRequest.type),
    mergeMap((action) =>
      BaseApiService.graphqlEntityCRUD('Maintenance')
        .delete({
          id: action.payload && action.payload.id,
        })
        .pipe(
          ...
          catchError(({ error }) => {
            ...
            return ofErrors.pipe(
              mergeMap((error) => [
                actions.safeDeleteFailure(error),
                globalAppErrorsActions.dispatchCustomAction(error),
              ]),
            );
          }),
 ...
 * @desc Redux-Observable Epic which allows to dispatch a custom action in "hospitalManagement" workflow.
 */
    ...
    mergeMap((action) => {
      ...
      return BaseApiService.genericDispatchCustomAction(params).pipe(
        ...
        catchError(({ error }) => {
          ...
          return ofErrors.pipe(
            mergeMap((error) => [
              actions.dispatchCustomActionFailure(error),
              globalAppErrorsActions.dispatchCustomAction(error),
            ]),
          );
        }),
...
/**
 * Get
 * @desc Redux-Observable Epic which allows to get an entity in "hospitalManagement" workflow.
 */
...
/**
 * Auto Complete
 * @desc Redux-Observable Epic which allows to auto-complete values in "hospitalManagement" workflow.
 */
    ...
    mergeMap((action) =>
      BaseApiService.graphqlEntityAutoComplete(action.payload.entity).pipe(
        ...
        catchError(({ error }) => {
          ...
          return ofErrors.pipe(
            mergeMap((error) => [
              actions.getAutoCompleteValuesFailure(error),
              globalAppErrorsActions.dispatchCustomAction(error),
            ]),
          );
        }),
...
/**
 * Get Widget Data
 * @desc Redux-Observable Epic which allows to get the data for a widget in "hospitalManagement" workflow.
 */
...
/**
 * Update
 * @desc Redux-Observable Epic which allows to update an entity in "hospitalManagement" workflow.
 */
...
/**
 * Bulk Update
 * @desc Redux-Observable Epic which allows to update entities in bulk in "hospitalManagement" workflow.
 */
